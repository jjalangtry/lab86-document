...
contextBridge.exposeInMainWorld('vault', {
  ...
  log: (level, message) => invoke('app:log', level, message),
  openLogs: () => invoke('app:logs'),
  readState: name => invoke('state:read', name),
  writeState: (name, value) => invoke('state:write', name, value),
  ...
  createNote: (folder, name, text, extension) => invoke('note:create', folder, name, text, extension),
  ...
  importImage: folder => invoke('attachment:import', folder),
  saveAttachment: (name, data, folder) => invoke('attachment:save', name, data, folder),
